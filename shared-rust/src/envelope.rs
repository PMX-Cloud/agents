...
mod tests {
    ...
    fn make_interop_key() -> (VerifyingKey, SigningKey) {
    ...
    fn make_signed_envelope(sk: &SigningKey, vk: &VerifyingKey) -> Envelope {
        let now = Utc::now();
        make_signed_envelope_at(sk, vk, now)
    }

    fn make_signed_envelope_at(sk: &SigningKey, vk: &VerifyingKey, now: DateTime<Utc>) -> Envelope {
        use chrono::Duration;
    ...
    fn interop_writes_rust_signed_envelope() {
        let (vk, sk) = make_interop_key();
        let issued_at = DateTime::parse_from_rfc3339("2026-01-01T00:00:00Z")
            .unwrap()
            .with_timezone(&Utc);
        let env = make_signed_envelope_at(&sk, &vk, issued_at);
        ...
        // Verify the deterministic fixture round-trips without coupling the
        // cross-language signature proof to the fixture's wall-clock age.
        ...
        use ed25519_dalek::Verifier;
        let canonical_back = env_back.canonical_bytes().unwrap();
        let signature_bytes = hex::decode(&env_back.signature).unwrap();
        let signature = Signature::from_slice(&signature_bytes).unwrap();
        vk_back
            .verify(&canonical_back, &signature)
    ...
    fn interop_go_signed_envelope() {
        let cbor_path = "../shared/envelope/testdata/envelope-v1.cbor";
        let pubkey_path = "../shared/envelope/testdata/pubkey.hex";

        // If the sibling Go package is unavailable, keep standalone crate tests usable.
        ...
        let canonical = env
            .canonical_bytes()
            .expect("canonicalize Go envelope in Rust");
        let signature_bytes = hex::decode(&env.signature).expect("decode Go signature");
        let signature = Signature::from_slice(&signature_bytes).expect("parse Go signature");
        vk.verify(&canonical, &signature)
            .expect("Go-signed envelope signature must verify in Rust");
        assert_eq!(env.audience, "pmx-network");
        assert_eq!(env.host, "aabbccdd");
    }
}
