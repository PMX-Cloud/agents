...
func Poll(ctx context.Context, ex storageexec.Interface, devices []string) (*PollResult, error) {
	...
	for _, dev := range devices {
		...
		if err != nil {
			...
			if errors.Is(err, storageexec.ErrExit) && (strings.Contains(stderr, "unavailable") || strings.Contains(stderr, "unsupported") || strings.Contains(stderr, "not support")) {
		...
		if err := json.Unmarshal(res.Stdout, &parsed); err != nil {
			entry.Status = "error"
			entry.Error = fmt.Sprintf("parse smartctl json: %v", err)
			out.Disks = append(out.Disks, entry)
			continue
		}
