...
func TestAdditionalProxMenuXCommandsBuildSafeShellSteps(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "lxc gpu attach",
			command:  "gpu.attach.lxc",
			payload:  map[string]any{"ctId": "201", "pciId": "0000:00:02.0", "type": "intel"},
			stepName: "lxc-gpu-attach",
			contains: "/dev/dri",
		},
		{
			name:     "nfs share create",
			command:  "nfs.share.create",
			payload:  map[string]any{"path": "/srv/shared/media", "clients": []string{"192.168.10.0/24"}, "options": []string{"rw", "sync"}},
			stepName: "nfs-share-create",
			contains: "exportfs -ra",
		},
		{
			name:     "samba share create",
			command:  "samba.share.create",
			payload:  map[string]any{"path": "/srv/shared/backups", "shareName": "backups", "users": []string{"ops"}, "readOnly": true},
			stepName: "samba-share-create",
			contains: "testparm -s",
		},
		{
			name:     "agent scripts sync",
			command:  "agent.scripts.sync",
			payload:  map[string]any{"scripts": []map[string]any{{"id": "pve-post-install", "version": "1.2.1", "url": "https://example.invalid/pve.sh", "checksum": "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"}}},
			stepName: "agent-scripts-sync",
			contains: "/var/lib/pmxcloud/scripts",
		},
		{
			name:     "hardware detect",
			command:  "hardware.detect",
			payload:  map[string]any{},
			stepName: "hardware-pci",
			contains: "lspci",
		},
		{
			name:     "disk inventory",
			command:  "disk.inventory",
			payload:  map[string]any{},
			stepName: "disk-block-inventory",
			contains: "lsblk -J -O",
		},
		{
			name:     "zfs status",
